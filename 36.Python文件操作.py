...
"""
# r+ 、w+ 、 a+ 带加号表示功能全，既能读又能写（区别在于指针到底指向不同）
# rb 、wb、ab 带b表示以二进制的形式对其进行操作，适合读取文本或二进制格式文件，如图片、音频、视频等格式
# rb+、wb+、ab+ 带+ 表示功能全，既能读又能写（区别在于指针到底指向不同）


# 九、seek()函数移动光标
"""
seek()函数用于移动文件中的指针到指定位置，语法如下：
      r => 文件开头
      w => 清空文件内容，指向文件头
      a => 文件尾
"""
...
# offset:开始的偏移量，也就是代表需要移动偏移的字节数。正数表示向后偏移，负数表示向前偏移
# whence:给offset参数一个定义，表示要从哪个位置开始偏移，默认值为0，表示文件开头。1代表从当前位置开始算起，2代表从文件末尾算起

"""实际工作中，seek()函数主要用于重置光标到起始位置：
f.seek(0)
或
f.seek(0,0)
"""
...
f.read(1)  # 读取1个字节，即第6个字节，值为b'5'
f.seek(-3,2)  # 从右向左移动3个字节
f.read(1)  # 读取1个字节，即倒数第3个字节，值为b'd'
